...
void init_I2C(void){ // will need to put pull up resistor on the SCL and SDA, unless it can be done on the board
...
void write_8bit_data(uint8_t data_8bit){
	TWDR = data_8bit;
	TWCR = (1<<TWINT) |	(1<<TWEN);
	while (!(TWCR &	(1<<TWINT))); //Wait for TWINT Flag set. This indicates that
								// address has been transmited and ACK/NACK has been received
}
uint8_t read_8bit_data_with_ack(void){
	 TWCR = (1<<TWINT)|(1<<TWEN)|(1<<TWEA);
	 while ((TWCR & (1<<TWINT)) == 0);
	 return TWDR;
}
uint8_t read_8bit_data_with_nack(void){
	TWCR = (1<<TWINT)|(1<<TWEN);
	while ((TWCR & (1<<TWINT)) == 0);
	return TWDR;
}
uint8_t read_I2C_status(void){
	uint8_t status;
